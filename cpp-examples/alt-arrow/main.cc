...
void doit(Server *server) {
  ...
  // 2. Add "X" column (type: int) to schema
  {
    auto priceMetadata = std::make_shared<arrow::KeyValueMetadata>();
    okOrThrow(DEEPHAVEN_EXPR_MSG(priceMetadata->Set("deephaven:type", "int")));
    auto priceField = std::make_shared<arrow::Field>("X",
        std::make_shared<arrow::Int32Type>(), true, std::move(priceMetadata));
  ...
  // 4. Populate X's column builder
  arrow::Int32Builder xBuilder;
  std::vector<int32_t> xs{254};
  ...
  // 5. Gather Arrow columns
  std::vector<std::shared_ptr<arrow::Array>> columns = {
      std::move(xCol)
  };

  // 6. Allocate an Arrow flight descriptor
  auto [tableTicket, fd] = server->newTicketAndFlightDescriptor();

  // 7. DoPut takes FlightCallOptions, which need to at least contain the Deephaven
  ...
  // 9. Make a RecordBatch containing both the schema and the data
