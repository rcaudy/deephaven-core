 ...
 */
public final class ParquetColumnRegionDouble<ATTR extends Values> extends ParquetColumnRegionBase<ATTR>
        implements ColumnRegionDouble<ATTR>, ParquetColumnRegion<ATTR> {

    public ParquetColumnRegionDouble(
    ...
    public RowSet binSearchMatch(
        ...
        return DoubleRegionBinarySearchKernel.binarySearchMatch(this, firstRowKey, lastRowKey, sortColumn, keys);
    }
    ...
    public boolean mightContain(@NotNull final Object[] keys) {
        ...
        final DoubleStatistics stats = columnChunkPageStore.getStatistics();
        ...
        final double[] typed = ArrayTypeUtils.getUnboxedDoubleArray(keys);
        try (final DoubleTimsortKernel.DoubleSortKernelContext<Any> sortContext =
                     DoubleTimsortKernel.createContext(typed.length)) {
            sortContext.sort(WritableDoubleChunk.writableChunkWrap(typed));
        }

        int firstNonNullValue = 0;
        if (typed[firstNonNullValue] == QueryConstants.NULL_DOUBLE) {
            ...
            while (firstNonNullValue < typed.length && typed[firstNonNullValue] == QueryConstants.NULL_DOUBLE) {
                firstNonNullValue++;
            }
