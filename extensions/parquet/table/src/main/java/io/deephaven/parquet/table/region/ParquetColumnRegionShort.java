 ...
 */
public final class ParquetColumnRegionShort<ATTR extends Values> extends ParquetColumnRegionBase<ATTR>
        implements ColumnRegionShort<ATTR>, ParquetColumnRegion<ATTR> {

    public ParquetColumnRegionShort(
    ...
    public RowSet binSearchMatch(
        ...
        return ShortRegionBinarySearchKernel.binarySearchMatch(this, firstRowKey, lastRowKey, sortColumn, keys);
    }
    ...
    public boolean mightContain(@NotNull final Object[] keys) {
        ...
        final short[] typed = ArrayTypeUtils.getUnboxedShortArray(keys);
        try (final ShortTimsortKernel.ShortSortKernelContext<Any> sortContext =
                     ShortTimsortKernel.createContext(typed.length)) {
            sortContext.sort(WritableShortChunk.writableChunkWrap(typed));
        }

        int firstNonNullValue = 0;
        if (typed[firstNonNullValue] == QueryConstants.NULL_SHORT) {
            ...
            while (firstNonNullValue < typed.length && typed[firstNonNullValue] == QueryConstants.NULL_SHORT) {
                firstNonNullValue++;
            }
